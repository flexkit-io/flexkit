...
const renderAttachmentImage = (url: string, filename: string | undefined, isGrid: boolean) =>
  isGrid ? (
    <img alt={filename || 'Image'} className="fk:size-full fk:object-cover" height={96} src={url} width={96} />
  ) : (
    <img
      alt={filename || 'Image'}
...
export const Attachments = ({ variant = 'grid', className, children, ...props }: AttachmentsProps) => {
  ...
  return (
    <AttachmentsContext.Provider value={contextValue}>
      <div
        className={cn(
          'fk:flex fk:items-start',
          variant === 'list' ? 'fk:flex-col fk:gap-2' : 'fk:flex-wrap fk:gap-2',
          variant === 'grid' && 'fk:ml-auto fk:w-fit',
          className
        )}
        {...props}
      >
...
export const AttachmentPreview = ({ fallbackIcon, className, ...props }: AttachmentPreviewProps) => {
  ...
  const iconSize = variant === 'inline' ? 'fk:size-3' : 'fk:size-4';

  const renderIcon = (Icon: typeof ImageIcon) => <Icon className={cn(iconSize, 'fk:text-muted-foreground')} />;
  ...
  const renderContent = () => {
    ...
    if (mediaCategory === 'video' && data.type === 'file' && data.url) {
      return <video className="fk:size-full fk:object-cover" muted src={data.url} />;
    }
...
export const AttachmentInfo = ({ showMediaType = false, className, ...props }: AttachmentInfoProps) => {
  ...
  return (
    <div className={cn('fk:min-w-0 fk:flex-1', className)} {...props}>
      <span className="fk:block fk:truncate">{label}</span>
      {showMediaType && data.mediaType && (
        <span className="fk:block fk:truncate fk:text-muted-foreground fk:text-xs">{data.mediaType}</span>
      )}
    </div>
  );
};
...
export const AttachmentRemove = ({ label = 'Remove', className, children, ...props }: AttachmentRemoveProps) => {
  ...
  return (
    ...
    >
      {children ?? <XIcon />}
      <span className="fk:sr-only">{label}</span>
    </Button>
  );
};
...
export const AttachmentHoverCardContent = ({
  ...
  <HoverCardContent align={align} className={cn('fk:w-auto fk:p-2', className)} {...props} />
);
...
export const AttachmentEmpty = ({ className, children, ...props }: AttachmentEmptyProps) => (
  <div
    className={cn('fk:flex fk:items-center fk:justify-center fk:p-4 fk:text-muted-foreground fk:text-sm', className)}
    {...props}
  >
