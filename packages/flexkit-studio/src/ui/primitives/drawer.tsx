...
>(({ className, children, ...props }, ref) => (
  <DrawerPortal>
    ...
    <DrawerPrimitive.Content
      className={cn(
        'fk-fixed fk-w-full fk-max-w-[clamp(40vw,95vw,60rem)] fk-right-0 fk-bottom-0 fk-z-50 fk-flex fk-h-full fk-flex-col !fk-transition-all fk-duration-500 fk-rounded-l-[20px] fk-border fk-border-border fk-bg-background',
        className
      )}
      ref={ref}
      {...props}
...
function DrawerHeader({ className, ...props }: React.HTMLAttributes<HTMLDivElement>): JSX.Element {
  return <div className={cn('fk-flex fk-gap-2.5 fk-p-4 fk-text-center sm:fk-text-left', className)} {...props} />;
}
